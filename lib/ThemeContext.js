...
function buildTokens(colors, isDark) {
  // Gradients - ALWAYS use these tokens, never hardcode gradient values!
  const gradients = {
    // Primary action gradient (buttons, CTAs) - mint → lavender
    ...
    // Subtle surface gradients (selected states, highlights)
    surface: `linear-gradient(135deg, ${colors.aurora.mint}20, ${colors.aurora.lavender}20)`,
    surfaceLight: `linear-gradient(135deg, ${colors.aurora.mint}10, ${colors.aurora.lavender}10)`,
    surfaceSubtle: `linear-gradient(135deg, ${colors.aurora.mint}15, ${colors.aurora.lavender}10)`,

    // Mint gradients (completed states, success) - same as primary for consistency
    mint: `linear-gradient(135deg, ${colors.aurora.mint}, ${colors.aurora.lavender})`,
    mintHorizontal: `linear-gradient(90deg, ${colors.aurora.mint}, ${colors.aurora.lavender})`,
    mintSurface: `linear-gradient(135deg, ${colors.aurora.mint}20, ${colors.aurora.lavender}20)`,

    // Rose accent gradient (chat bubbles, special accents)
    rose: `linear-gradient(135deg, ${colors.aurora.lavender}, ${colors.aurora.rose})`,
    roseHorizontal: `linear-gradient(90deg, ${colors.aurora.lavender}, ${colors.aurora.rose})`,
    roseSurface: `linear-gradient(135deg, ${colors.aurora.lavender}20, ${colors.aurora.rose}20)`,
...
export function ThemeProvider({ children }) {
  ...
  // Build complete tokens - use light mode as default before loaded
  const colors = isDarkMode ? darkColors : lightColors;
  const tokens = buildTokens(colors, isDarkMode);

  // Always render with default (light) tokens to prevent app from hanging
  // The slight flash is better than a stuck loading screen
  return (
